...
def get_T_natural_to_torch(shape):
    #T = np.eye(4)
    T = torch.eye(4)
    for i, dim in enumerate(shape[-4:][::-1]):
        if dim == 1:
            dim = float('nan') #np.nan
        T[i, i] = 2 / (dim - 1)
        T[i, -1] = -1
    return T
...
class DummySeries(object):
    def __init__(self, images=None, T_pix2world=None, T_world2pix=None, T_nat2torch=None, T_torch2nat=None):
        self.images = images
...
def create_nifti_series(nii_file, affine, force_v1=False):
    if isinstance(nii_file, str):
        nifti_image = nib.load(nii_file)
    else:
        images = nii_file

    T_pix2world = affine.clone()
    T_world2pix = torch.linalg.inv(T_pix2world)
    # to DHW
    T_nat2torch = get_T_natural_to_torch(images.shape).unsqueeze(0).to(T_pix2world.device)# WHD->DHW??
    T_torch2nat = get_T_torch_to_natural(images.shape).unsqueeze(0).to(T_pix2world.device)
    # spacing_xyz
    spacing = torch.linalg.norm(affine[:3, :3], axis=0)
    ...
    series.affine_itk = affine #nifti_image = nib.load(nii_file)

    return series,images.shape

    # images = np.asanyarray(nifti_image.data)
...
class RegistrationByWorldCoordinate(object):
    ...
    def get_theta_1_to_2(series1, series2):
        T = series2.T_nat2torch @ series2.T_world2pix @ series1.T_pix2world @ series1.T_torch2nat #series2.T_world2pix @ series1.T_pix2world  #series2.T_nat2torch @ series2.T_world2pix @ series1.T_pix2world @ series1.T_torch2nat
        return T
    ...
    def image1_to_2(self, series1, series2, src=None,mode="bilinear"):
        ...
        grid = F.affine_grid(theta, tuple(series2.shape), align_corners=False)#(1, 1) + tuple(tar.shape)
        kwargs = {"align_corners": False} #if mode == "bilinear" else {}
        out = F.grid_sample(src, grid, mode=mode, **kwargs)#src[None, None,:,:]
        return out.to(series2.images.device)
...
def image_register(series1,series2):
    registrator = RegistrationByWorldCoordinate(device=series2.images.device)

    # series1,pd_fs_shape = create_nifti_series(f1,affine1, force_v1=True)
    # series2,pd_shape = create_nifti_series(f2,affine2, force_v1=True)  
    # print(series2.shape)
    if (torch.abs(series1.affine_itk.to(series2.affine_itk.device) - series2.affine_itk) == 0).all():  # identical affine matrix 判断仿射矩阵是否相同，如果相同就不需要配准了
